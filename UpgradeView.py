...
class UpgradeView(arcade.View):
    ...
    def on_draw(self):
        ...
        arcade.draw_text(f"Fire rate: {7-self.player.fireRate/5}" + ("Maxed" if self.player.maxed[4] else f"->{8-self.player.fireRate/5}"), parameters.SCREEN_WIDTH/2, parameters.SCREEN_HEIGHT/2, arcade.color.GRAY if self.player.maxed[4] else arcade.color.WHITE, 20, anchor_x="center")
