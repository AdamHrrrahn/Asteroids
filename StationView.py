...
class StationView(arcade.View):
    def setViews(self, game, player, upgrade):
        self.gameView = game
    ...
    def on_draw(self):
        ...
        arcade.draw_text("Quit", parameters.SCREEN_WIDTH/2, parameters.SCREEN_HEIGHT/2-120, arcade.color.WHITE, 20, anchor_x="center")
    ...
    def repair_button(self):
        if self.player.health < self.player.maxHealth :
            self.player.wallet -= 10*(self.player.maxHealth - self.player.health)
